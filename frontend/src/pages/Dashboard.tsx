...
import type { DashboardStats, Habit, HabitEntry, AIInsight } from '../types';
...
export default function Dashboard() {
  ...
  const { data: aiInsight, loading: insightLoading } = useApi<AIInsight>('/ai/daily-insight');
  ...
  const fetchTodayEntries = useCallback(async () => {
    try {
      const data = await apiRequest<(HabitEntry & { habit_name: string; category: string })[]>(`/habit-entries?date=${today}`);
  ...
  useEffect(() => {
    fetchHabits();
    fetchTodayEntries();
  }, [fetchHabits, fetchTodayEntries]);

  const toggleHabit = async (habitId: number, completed: boolean) => {
    try {
      await apiRequest('/habit-entries', {
        method: 'POST',
        body: JSON.stringify({
          habit_id: habitId,
          date: today,
        ...
        <div className="space-y-6">
          <Card className="p-6">
            ...
            {insightLoading ? (
              ...
              <div className="bg-blue-50 border-blue-200 border-2 p-4 rounded-xl">
                <p className="text-sm text-gray-800 leading-relaxed">
                  Great job! You're building positive momentum. Keep going with your habits today! 🌟
                </p>
              </div>
            )}
          </Card>
